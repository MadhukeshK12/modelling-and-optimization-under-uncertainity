...
% D=2 for Function 7
%[sp_x,sp_val,sp_ex_flag,sp_op] = particleswarm(@(x)benchmark_func(x,7),2,[-100,-100],[100,100],options);
% D=10 for Function 7
[sp_x,sp_val,sp_ex_flag,sp_op] = particleswarm(@(x)benchmark_func(x,7),2,[-100,-100],[100,100],options)
sp_main_x(i,:) = sp_x;
sp_main_val(i) = sp_val;
sp_main_exit_flag(i) = sp_ex_flag;
sp_main_op(i) = sp_op;
% saving Plot
fname = sprintf('filenameP(%d).fig', i) ;
savefig(fname)
end
%% PS Calculations
